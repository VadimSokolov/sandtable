...
span (one-to-eight) on-platform autonomy wins at every communications level, by
\cpDeltaHiSpanClear{}~percentage points even in the clear. Second, route choice traces a clean
speed-survivability frontier: a defilade route reaches \ucThreeSuccCover\% mission success against
\ucThreeSuccFast\% for a fast exposed corridor, at \ucThreeTempoRatio$\times$ the time. Third, a
recon swarm cueing near-blind shooters lifts success from \ucFiveSuccBlind\% (no recon) to
\ucFiveSuccBestClear\%, but jamming degrades the sensor-to-shooter chain and cuts that to
\ucFiveSuccBestSevere\%. \sandtable{} is offered as the fast inner-loop companion to ProjectGL for
Army mission engineering.
\end{abstract}
...
are within Monte-Carlo noise, so the robust claim is the direction of the boundary, not its precise
cell. The Lanchester check validates the aggregate attrition law but not the spatial or command

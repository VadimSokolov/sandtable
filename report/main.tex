...
communications. The subsections below describe each layer and the two properties that make the model
fit for a search loop.
...
by dropping relayed contacts. The six rungs raise one-way tasking latency from 0 to 16 time steps
and message-drop probability from 0 to 0.55, spanning a benign link to a heavily contested one; the
rung values are set parameters, not measured link data. Because supervisory autonomy does not depend
on the link while direct control does, the ladder is the axis along which the two modalities
separate.
